...
async function updateApplication(req, res, next) {
  try {
    ...
    if (req.query && req.query.type === 'swimlane' && req.controllerData.los_statuses) {
        // if (req.body && req.body.source_idx !== req.body.destination_idx)
        //     updateOptions = {
      ...
      updateOptions = {
        ...
        const response = await fetch(`http://192.168.43.36:10000/v1/rest/digify/can_move_status_of_app?application_id=${req.params.id}&source_index=${application.status}&destination_index=${req.body.status}`);
