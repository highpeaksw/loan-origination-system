  ...
  try {
    ...
    if (req.query && req.query.type === 'swimlane' && req.controllerData.los_statuses) {
      ...
      updateOptions = {
        ...
        const response = await fetch(`http://164.52.220.31:10000/v1/rest/digify/can_move_status_of_app?application_id=${req.params.id}&source_index=${application.status}&destination_index=${req.body.status}`);
